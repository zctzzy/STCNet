...
# from JSAC.Github_Version.utils.dataset import read_data
# from JSAC.Github_Version.utils.model import DenseNet
sys.path.append('../../')
...
if __name__ == '__main__':
    ...
    if opt.cluster > 1:
        labels_df = pd.read_csv('../data/cluster_label_20.csv', header=None)
    ...
    # row, col = 10, 18
    # plt.plot(prediction_ct[:, :, row, col], 'r-', label='Prediction')
    # plt.plot(truth_ct[:, :, row, col], 'k-', label='Ground Truth')

    # import h5py
    # f = h5py.File(opt.traffic + '_results.h5', 'w')
